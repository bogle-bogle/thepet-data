...
file_path = 'C:/Users/KOSA/Desktop/thepet_data/app/dataset/ingredient_counts.csv'  # 경로를 실제 파일 경로로 변경해야 합니다.
...
for ingredient in sorted_ingredients_list:
    if ingredient not in already_clustered:
        similar_ingredients = substring_similarity(ingredient, sorted_ingredients_list)
...
# 군집 출력!!!!
example_clusters = {k: ingredient_clusters[k] for k in list(ingredient_clusters.keys())}
...
fasttext_clustered_model = FastText(clustered_ingredients, vector_size=300, window=5, min_count=1, workers=4)
